    ...
    <v-container>
        <v-row v-bind:class="{ 'mb-2': !section.MA_EMA_ADX.display }">
            <v-app-bar dense>
                ...
                <v-btn
                    v-if="section.MA_EMA_ADX.loaded"
                    icon
                    @click="
                        section.MA_EMA_ADX.display = !section.MA_EMA_ADX.display
                    "
                >
                    <v-icon v-if="section.MA_EMA_ADX.display">
        ...
        <v-row v-bind:class="{ 'mb-2': !section.RSI_ADX.display }">
            <v-app-bar dense>
                ...
                <v-btn
                    v-if="section.RSI_ADX.loaded"
                    icon
                    @click="section.RSI_ADX.display = !section.RSI_ADX.display"
                >
                    <v-icon v-if="section.RSI_ADX.display">
                        mdi-unfold-less-horizontal
                    </v-icon>
                    <v-icon v-else>mdi-unfold-more-horizontal</v-icon>
                </v-btn>
        ...
        <v-slide-y-reverse-transition>
            <RSI-ADX-List
                v-if="recommendationView == 'list' && section.RSI_ADX.display"
                :by_RSI_ADX="by_RSI_ADX"
                :sparkline="sparkline"
            ></RSI-ADX-List>

            <RSI-ADX-Tile
                v-if="recommendationView == 'tile' && section.RSI_ADX.display"
                :by_RSI_ADX="by_RSI_ADX"
                :sparkline="sparkline"
            ></RSI-ADX-Tile>
        </v-slide-y-reverse-transition>
        ...
        <v-row>
            <v-app-bar dense>
                ...
                <v-btn
                    v-if="section.RSI_MACD.loaded"
                    icon
                    @click="
                        section.RSI_MACD.display = !section.RSI_MACD.display
                    "
                >
                    <v-icon v-if="section.RSI_MACD.display">
                        mdi-unfold-less-horizontal
                    </v-icon>
                    <v-icon v-else>mdi-unfold-more-horizontal</v-icon>
                </v-btn>
        ...
        <v-slide-y-reverse-transition>
            <RSI-MACD-List
                v-if="recommendationView == 'list' && section.RSI_MACD.display"
                :by_RSI_MACD="by_RSI_MACD"
                :sparkline="sparkline"
            ></RSI-MACD-List>

            <RSI-MACD-Tile
                v-if="recommendationView == 'tile' && section.RSI_MACD.display"
                :by_RSI_MACD="by_RSI_MACD"
                :sparkline="sparkline"
            ></RSI-MACD-Tile>
        </v-slide-y-reverse-transition>
            ...
            axios
                ...
                .then((response) => {
                    ...
                    recommendations.forEach((stock) => {
                        ...
                        let ten_reverse_EMA_high = reverse_EMA_high.filter(
                            (a, i) => {
                                return i >= 0 && i <= 15;
                            }
                        );
                        let ten_reverse_EMA_hlc3 = reverse_EMA_hlc3.filter(
                            (a, i) => {
                                return i >= 0 && i <= 15;
                            }
                        );
                        let ten_reverse_EMA_low = reverse_EMA_low.filter(
                            (a, i) => {
                                return i >= 0 && i <= 15;
                            }
                        );
                    ...
                    this.by_MA_EMA_ADX = recommendations.sort(
                        (a, b) =>
                            parseFloat(b.traded_shares) -
                            parseFloat(a.traded_shares)
                    );
            ...
            axios
                ...
                .then((response) => {
                    ...
                    data.buyRecommendations.forEach((stock, i) => {
                        ...
                        data["buyRecommendations"][i].reverse_RSI =
                            ten_reverse_RSI.map((n) => n.toFixed(2));
                        data["buyRecommendations"][i].reverse_ADX =
                            ten_reverse_ADX.map((n) => n.toFixed(2));
            ...
            axios
                ...
                .then((response) => {
                    ...
                    Object.keys(recommendations).forEach((symbol) => {
                        ...
                        recommendations[symbol].reverse_RSI =
                            ten_reverse_RSI.map((n) => n.toFixed(2));
                        recommendations[symbol].reverse_MACD =
                            ten_reverse_MACD.map((n) => n.toFixed(2));
